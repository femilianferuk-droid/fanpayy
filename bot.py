...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем актуальный курс из базы
    ...
    await update.message.reply_text(
        f"🌟 Добро пожаловать в бот для покупки Telegram Stars!\n\n"
        f"Курс: 1 звезда = {current_rate} рубль\n"
        f"Минимальная покупка: 50 звезд\n"
        f"Максимальная покупка: 100000 звезд",
...
async def show_admin_panel(query, context):
    ...
    # Получаем актуальный курс для отображения в админке
    current_rate = db.get_star_rate()
...
async def get_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Расчет стоимости в рублях с использованием актуального курса из базы
...
async def handle_payment_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем актуальный курс для отображения
    current_rate = db.get_star_rate()
    ...
    if query.data == "payment_sbp":
        await query.edit_message_text(
            ...
            f"💎 Курс: 1 звезда = {current_rate} руб.\n\n"
...
async def change_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Сохраняем новый курс в базу
        db.set_star_rate(new_rate)
        
        await update.message.reply_text(
            f"✅ Курс обновлен!\n"
            f"Новый курс: 1 звезда = {new_rate} руб.\n\n"
            f"Теперь во всех сообщениях бота будет отображаться обновленный курс."
        )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Справка по боту"""
    # Получаем актуальный курс для справки
    current_rate = db.get_star_rate()
    ...
    await update.message.reply_text(
        ...
        "/buy - Купить звезды\n"
        "/help - Помощь\n\n"
        f"Курс: 1 звезда = {current_rate} рубля\n"
